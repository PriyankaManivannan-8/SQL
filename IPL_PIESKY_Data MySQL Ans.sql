...
-- percentage.
select b.bidder_id,b.no_of_matches,
sum(case when a.BID_STATUS ='Won' then 1
else 0 end)bidding_status,((sum(case when a.BID_STATUS ='Won' then 1
else 0 end)/no_of_matches) * 100) as winning_percentage
from ipl_bidding_details a
right join ipl_bidder_points b on a.bidder_id = b.bidder_id
group by a.bidder_id
order by winning_percentage desc;

-- 2.Display the number of matches conducted at each stadium with stadium name, city from 
-- the database.
select a.stadium_id,b.stadium_name,b.city,count(distinct a.match_id)as No_of_matches
from ipl_match_schedule a
join ipl_stadium b on a.stadium_id=b.stadium_id
group by a.stadium_id;

-- 3) In a given stadium, what is the percentage of wins by a team which has won the toss?
-- ipl match. -- wrong
-- won toss Team/total match in that stadium team

with tab as
(select * from (select MATCH_ID, TEAM_ID1 as TEAM_ID,if(TOSS_WINNER in (1,2),if(TOSS_WINNER = 1,TEAM_ID1,TEAM_ID2),MATCH_WINNER) TOSS_WINNER, 
	if(MATCH_WINNER in (1,2),if(MATCH_WINNER = 1,TEAM_ID1,TEAM_ID2),MATCH_WINNER) MATCH_WINNER 
from ipl_match) t1 
union 
(select MATCH_ID, TEAM_ID2 as TEAM_ID,if(TOSS_WINNER in (1,2),if(TOSS_WINNER = 1,TEAM_ID1,TEAM_ID2),MATCH_WINNER) TOSS_WINNER, 
	if(MATCH_WINNER in (1,2),if(MATCH_WINNER = 1,TEAM_ID1,TEAM_ID2),MATCH_WINNER) MATCH_WINNER 
from ipl_match))

select STADIUM_NAME,TEAM_ID,TOSS_WINNER,MATCH_WINNER,
round((sum(TOSS_WINNER = MATCH_WINNER and TEAM_ID = MATCH_WINNER)/count(*))*100,2) 
'winning Percentage where toss_match_won'
from tab natural join 
(select * from ipl_match_schedule where STATUS != 'Cancelled') b natural join ipl_stadium
group by STADIUM_ID,TEAM_ID
order by STADIUM_NAME,TEAM_ID;


-- 4.Show the total bids along with bid team and team name.

select a.bidder_id,a.bid_team,count(a.bid_team) as bidding_count,b.team_name 
from ipl_bidding_details a
left join ipl_team b on a.bid_team = b.team_Id
group by a.bidder_id,a.bid_team;

-- 5.	Show the team id who won the match as per the win details.-- wrong
select match_id,team_name as match_winner from ipl_match a
join ipl_team b on a.match_winner=b.team_id;
--
select  TEAM_ID from ipl_team_standings where REMARKS is not null ;

-- 6.Display total matches played, total matches won and total matches lost by team along 
-- with its team name.
select a.team_id,a.team_name,sum(b.matches_played),sum(b.matches_won),sum(b.matches_lost) 
from ipl_team a
join ipl_team_standings b on a.team_id=b.team_id
group by team_id;

-- 7.	Display the bowlers for Mumbai Indians team.
select a.player_id from ipl_team_players a
join ipl_team b on a.team_id = b.team_id
where team_name='Mumbai Indians' and a.player_role = 'Bowler';
-- or
select iptp.PLAYER_ID,ippl.PLAYER_NAME from ipl_team_players iptp join ipl_team ipt 
 on iptp.TEAM_ID=ipt.TEAM_ID
 join ipl_player ippl
 on ippl.PLAYER_ID=iptp.PLAYER_ID
 where TEAM_NAME='Mumbai Indians'and PLAYER_ROLE='Bowler';

-- 8.	How many all-rounders are there in each team, Display the teams with more than 4 
-- all-rounder in descending order.
select team_id,count(player_role) as all_rounder_counts from ipl_team_players 
